...
import static jlll.common.Jlll.eval;
import static jlll.common.Jlll.prepare;
...
{	
	...
	@Test public void testInit() throws Exception
	{
		...
		catch(JlllException exc) {catched=true;}
		assertTrue(catched);
		new Primitive("sql-get-connection", env)
		{
			/**
	...
	@Test public void testMPS() throws Exception
	{
		eval("(define param.name1 \"aaa\") ",env);
		eval("(define id1 2) ",env);
		assertEquals(1, eval("(caar (sql-execute-mps \"select id from jlll_testcase where name=$param.name1$ \"))",env));
